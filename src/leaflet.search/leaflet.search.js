...
(function (factory) {
  ...
  L.Control.Search = L.Control.extend({
    ...
    _createInput: function (text, className) {
      ...
      .on(input, 'keyup paste', async function (e) {
          // Clear the debounce timer
          clearTimeout(timer);
  
          const dynamicInput = e.target.value.trim().toLowerCase(); // Trim whitespace from the input
  
          // Set a new debounce timer to call getFilteredFeatures after 1 second
          timer = setTimeout(() => {
              // Clear existing layers from the group before adding the new one
              searchLayer.clearLayers();
              // Call getFilteredFeatures function only if the input is not empty
              if (dynamicInput) {
                  getFilteredFeatures(dynamicInput);       
              }
              self.options.propertyName = `${searchCapaSelected[1]}`;
